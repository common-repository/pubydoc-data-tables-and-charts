...
class UtilsPyt {
	public static function jsonEncode( $arr, $ent = false ) {
		return ( is_array($arr) || is_object($arr) ) ? jsonEncodeUTFnormalPyt($arr, $ent) : jsonEncodeUTFnormalPyt(array());
	}
	...
	public static function isThisCommercialEdition() {
		foreach (FramePyt::_()->getModules() as $m) {
	...
	public static function isMobile() {
		importClassPyt('Mobile_Detect', PYT_HELPERS_DIR . 'mobileDetect.php');
	...
	public static function isPluginsPage() {
		return ( basename(ReqPyt::getVar('SCRIPT_NAME', 'server')) === 'plugins.php' );
	}
	...
	public static function getReferalUrl() {
		// Simple for now
		return ReqPyt::getVar('HTTP_REFERER', 'server');
	}
	...
	public static function lettersToNumbers( $data ) {
		$letters = range('A', 'Z');
	...
	public static function getArrayValue( $params, $name, $default = '', $typ = 0, $arr = false, $zero = false ) {
		...
		if (empty($params[$name])) {
			return ( $zero && ( '0' === $params[$name] ) ) ? '0' : $default;
		}
		$value = $params[$name];
		if (1 == $typ) {
			if (!is_numeric($value)) {
