...
class ValidatorPyt {
	...
	public static function validate( $field, $table = false ) {
		self::$errors = array();
		if (is_object($field) && get_class($field) != 'FieldPyt') {
			$value = $field;
			$field = new FieldPytPyt('noMatter');
			$field->label = $label;
		...
		if (!empty($field->validate)) {
			foreach ($field->validate as $v) {
				if (method_exists('ValidatorPyt', $v)) {
					self::$v($field);
				}
			}
		}
		if (method_exists('ValidatorPyt', $field->type)) {
			$validate = $field->type;
			self::$validate($field);
		}
		if ($field->maxlen) {
		...
		if (is_object($table) && get_class($table) != 'TablePyt' && isset($table->lists[$field->name])) {
	...
	public static function getProductValidationMethods() {
		$res = array();
		$all = get_class_methods('ValidatorPyt');
		foreach ($all as $m) {
