...
class DbPyt {
	...
	public static function get( $query, $get = 'all', $outputType = ARRAY_A ) {
		...
		self::$query = $query;
		$wpdb->pyt_prepared_query = $query;
		switch ($get) {
			case 'one':
				$res = $wpdb->get_var($wpdb->pyt_prepared_query);
				break;
			case 'row':
				$res = $wpdb->get_row($wpdb->pyt_prepared_query, $outputType);
				break;
			case 'col':
				$res = $wpdb->get_col($wpdb->pyt_prepared_query);
				break;
			case 'all':
			default:
				$res = $wpdb->get_results($wpdb->pyt_prepared_query, $outputType);
				break;
		}
		return $res;
	}
	...
	public static function query( $query ) {
		global $wpdb;
		$wpdb->pyt_prepared_query = self::prepareQuery($query);
		return ( $wpdb->query($wpdb->pyt_prepared_query) === false ? false : true );
	}
	/**
	 * Get last insert ID
	 *
	 ...
	 * #__  Worwpfess prefix
	 * ^__  Store plugin tables prefix (@see PYT_DB_PREF if config.php)
	 *
	...
	public static function prepareQuery( $query ) {
		...
		return str_replace(
				array('#__', '^__', '@__'), 
				array($wpdb->prefix, PYT_DB_PREF, $wpdb->prefix . PYT_DB_PREF),
				$query);
	}
	...
	public static function timeToDate( $timestamp = 0 ) {
		if ($timestamp) {
			if (!is_numeric($timestamp)) {
				$timestamp = dateToTimestampPyt($timestamp);
			}
	...
	public static function dateToTime( $date ) {
		...
		if (strpos($date, PYT_DATE_DL)) {
			return dateToTimestampPyt($date);
		}
		$arr = explode('-', $date);
		return dateToTimestampPyt($arr[2] . PYT_DATE_DL . $arr[1] . PYT_DATE_DL . $arr[0]);
	}
