...
class HtmlPyt {
	...
	public static $colClasses = array(
		'standart' => array('label' => 'col-3 col-xl-2', 'info' => 'col-2 col-sm-1', 'values' => 'col-6 col-sm-8 col-xl-9', 'full' => 'col-12'),
	...
	public static function blockClasses( $type ) {
		echo esc_attr('options-' . $type . ' ' . UtilsPyt::getArrayValue(self::$colClasses[self::$colsType], $type));
	}
	public static function echoEscapedHtml( $html ) {
		add_filter('esc_html', array('HtmlPyt', 'skipHtmlEscape'), 99, 2);
		echo esc_html($html);
		remove_filter('esc_html', array('HtmlPyt', 'skipHtmlEscape'), 99, 2);
	}
	...
	public static function inputShortcode($name, $params = array()) {
		$value = $params['value'];
		self::input('', array('value' => $value, 'attrs' => 'readonly class="pubydoc-flat-input pubydoc-nosave pubydoc-shortcode pubydoc-width'. (strlen($value) <= 20 ? 200 : 300) .'"'));
	}
	...
	public static function selectbox( $name, $params = array('attrs' => '', 'options' => array(), 'value' => '') ) {
		$attrs = UtilsPyt::getArrayValue($params, 'attrs');
		//$params['attrs'] .= self::_dataToAttrs($params);
		if (UtilsPyt::getArrayValue($params, 'required', false)) {
			$attrs .= ' required ';
		}
		...
		echo '>';
		$default = UtilsPyt::getArrayValue($params, 'default');
		if (!empty($default)) {
			echo '<option value="">' . esc_html($default) . '</option>';
		}
		$existValue = isset($params['value']);
		$keyValue = UtilsPyt::getArrayValue($params, 'key') == 'value';
	...
	public static function selectlist( $name, $params = array('attrs'=>'', 'size'=> 5, 'options' => array(), 'value' => '') ) {
		...
		echo '<select multiple="multiple" class="pubydoc-chosen" size="' . esc_attr($params['size']) . '" name="' . esc_attr($name) . '" ';
		if (!empty($params['attrs'])) {
			self::echoEscapedHtml($params['attrs']);
		}
		echo '>';
	...
	public static function file( $name, $params = array() ) {
		...
		//self::input($name, $params);
		$id = ( empty($params['id']) ? self::nameToClassId($name) . mt_rand(9, 9999) : $params['id'] );
		echo '<div class="pubydoc-inputfile">
  			<input type="file" id="' . esc_attr($id) . '" name="' . esc_attr($name) . '">
  			<label for="' . esc_attr($id) . '">
    			<div class="pubydoc-namefile"></div>
    			<div class="button buttonfile">
      				<i class="fa fa-upload" aria-hidden="true"></i>' . esc_html__('Select File', 'publish-your-table') . '
      			</div>
  			</label>
		</div>';
	}
	...
	public static function colorPicker( $name, $params = array('value' => '', 'label' => '') ) {
		...
		echo '<div class="pubydoc-color-picker">
            <div class="pubydoc-color-wrapper">
            	<div class="pubydoc-color-preview"></div>
            </div>
            <input type="text" class="pubydoc-color-input" name="' . esc_attr($name) . '" value="' . esc_attr($value) . '"';
        if (!empty($params['attrs'])) {
			self::echoEscapedHtml($params['attrs']);
		}
		echo '>';
	...
	public static function selectIcon( $name, $params ) {
		echo '<div class="button chooseLoaderIcon">' . esc_html__('Choose Icon', 'publish-your-table') . '</div>';
	}
	public static function proOptionLink( $url = '', $label = '' ) {
		echo '<a href="' . ( empty($url) ? UriPyt::generatePluginLink() : esc_url($url) ) . '" target="_blank" class="pubydoc-prolink">' . ( empty($label) ? esc_html__('PRO', 'publish-your-table') : esc_html($label)) . '</a>';
	}
	public static function selectFontList( $name, $params = array('attrs' => '', 'value' => '') ) {
		$attrs = UtilsPyt::getArrayValue($params, 'attrs');
		$value = UtilsPyt::getArrayValue($params, 'value');

		echo '<select name="' . esc_attr($name) . '" ';
		if (!empty($params['attrs'])) {
			self::echoEscapedHtml($params['attrs']);
		}
		echo '><option value="">' . esc_html__('Default', 'publish-your-table') . '</option>';

		$standart = DispatcherPyt::applyFilters('getFontsList', array(), 'standart');
		$fonts = array_merge($standart, DispatcherPyt::applyFilters('getFontsList', array(), ''));
		natsort($fonts);
