...
abstract class ModulePyt extends BaseObjectPyt {
	...
	public function __construct( $d) {
		...
		if (isset($d['ex_plug_dir']) && !empty($d['ex_plug_dir'])) {
			...
			$this->setExternalDir( UtilsPyt::getExtModDir($d['ex_plug_dir']) );
			$this->setExternalPath( UtilsPyt::getExtModPath($d['ex_plug_dir']) );
		}
	}
	...
	public function getModDir() {
		if (empty($this->_externalDir)) {
			return PYT_MODULES_DIR . $this->getCode() . DS;
		} else {
	...
	public function getModPath() {
		if (empty($this->_externalPath)) {
			return PYT_MODULES_PATH . $this->getCode() . '/';
		} else {
	...
	protected function _createController() {
		...
		if (file_exists($this->getModDir() . 'controller.php')) {
			$className = '';
			if (importPyt($this->getModDir() . 'controller.php')) {
				$className = toeGetClassNamePyt($this->getCode() . 'Controller');
			}
			if (!empty($className)) {
	...
	protected function _createHelper() {
		...
		if (file_exists($this->getModDir() . 'helper.php')) {
			...
			importClassPyt($helper, $this->getModDir() . 'helper.php');
