...
class ResponsePyt {
	...
	public function ajaxExec( $forceAjax = false ) {
		$isAjax = $this->isAjax();
		$redirect = ReqPyt::getVar('redirect');
		if (count($this->errors) > 0) {
			$this->error = true;
		}
		if ($isAjax || $forceAjax) {
			HtmlPyt::echoEscapedHtml(jsonEncodeUTFnormalPyt($this));
			exit();
		}
		return $this;
	}
	public function mainRedirect( $redirectUrl = '' ) {
		$redirectUrl = empty($redirectUrl) ? PYT_SITE_URL : $redirectUrl;
		$redirectData = array();
		if (!empty($this->errors)) {
			$redirectData['pytErrors'] = $this->errors;
		}
		if (!empty($this->messages)) {
			$redirectData['pytMsgs'] = $this->messages;
		}
		return redirectPyt( $redirectUrl . ( strpos($redirectUrl, '?') ? '&' : '?' ) . http_build_query($redirectData) );
	}
	...
	/**
	 * Alias for ResponsePyt::addError, @see addError method
	 */
