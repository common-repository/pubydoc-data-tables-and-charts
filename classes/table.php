...
abstract class TablePyt {
	...
	public static function getInstance( $table = '' ) {
		...
		if (!isset($instances[$table])) {
			$class = 'table' . strFirstUpPyt($table) . strFirstUpPyt(PYT_CODE);
	...
	public function isRealTable() {
		return DbPyt::get("SHOW TABLES LIKE '" . $this->_table . "'");
	}
	public function bulkUpdate($set, $where) {
		return DbPyt::query('UPDATE ' . $this->_table . ' SET ' . $set . ' WHERE ' . $where);
	}
			...
			if ($id) {
				...
				foreach ($data as $field) {
					...
					foreach ($field as $k => $v) {
						if (isset($this->_fields[$k])) {
							$row[$k] = toeCreateObjPyt('FieldPyt', array(
									$this->_fields[$k]->name,
									$this->_fields[$k]->html,
									$this->_fields[$k]->type,
	...
	public function getTable( $transform = false ) {
		if ($transform) {
			return DbPyt::prepareQuery($this->_table);
		} else {
			return $this->_table;
		}
	}
	...
	public function get( $fields = '*', $where = '', $tables = '', $return = 'all' ) {
		...
		return DbPyt::get($query, $return);
	}
	public function store( $data, $method = 'INSERT', $where = '' ) {
		$this->_clearErrors();
		$method = strtoupper($method);
		if ($this->_escape) {
			$data = DbPyt::escape($data);
		}
		$query = '';
		switch ($method) {
			case 'INSERT':
		...
		if (empty($fields)) {
			$this->_addError(esc_html__('Nothing to update', 'publish-your-table'));
			return false;
		}
		...
		if (DbPyt::query($query)) {
			...
			$this->_addError(PYT_TEST_MODE ? DbPyt::getError() : esc_html__('Database error. Please contact your developer.', 'publish-your-table'));
		}
		return false;
	}
	public function bulkInsert( $data ) {
		...
		if (empty($data)) {
			$this->_addError(esc_html__('Nothing to insert', 'publish-your-table'));
			return false;
		}
		...
		if (!DbPyt::query(substr($query, 0, -1))) {
			$this->_addError(PYT_TEST_MODE ? DbPyt::getError() : esc_html__('Database error. Please contact your developer.', 'publish-your-table'));
			return false;
		}
		return true;
	...
	/**
	 * Add new FieldPytPyt for children table (@see class field)
	 *
	...
	protected function _addField( $name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '' ) {
		$this->_fields[$name] = toeCreateObjPyt('FieldPyt', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
		return $this;
	}
	/**
	...
	public function exists( $value, $field = '' ) {
		...
		return DbPyt::get('SELECT ' . $this->_id . ' FROM ' . $this->_table . ' WHERE ' . $field . ' = "' . $value . '"', 'one');
	}
			...
			switch ($f->type) {
				case 'date':
					if ('0000-00-00' == $d[$key] || empty($d[$key])) {
						...
						$d[$key] = gmdate(PYT_DATE_FORMAT, DbPyt::dateToTime($d[$key]));
					}
					break;
				case 'int':
				case 'tinyint':
	...
	public function getLastInsertID() {
		return DbPyt::get('SELECT MAX(' . $this->_id . ') FROM ' . $this->_table, 'one');
	}
