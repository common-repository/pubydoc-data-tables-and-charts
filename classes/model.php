...
abstract class ModelPyt extends BaseObjectPyt {
	...
	public function getModule() {
		return FramePyt::_()->getModule( $this->_code );
	}
	...
	protected function _buildQuery( $table = null ) {
		if (!$table) {
			$table = FramePyt::_()->getTable( $this->_tbl );
		}
	...
	public function removeGroup( $ids ) {
		...
		if (!empty($ids)) {
			if (FramePyt::_()->getTable($this->_tbl)->delete(array('additionalCondition' => 'id IN (' . implode(',', $ids) . ')'))) {
				$this->_dataRemove($ids);
				return true;
			} else {
				$this->pushError(esc_html__('Database error detected', 'publish-your-table'));
			}
		} else {
			$this->pushError(esc_html__('Invalid ID', 'publish-your-table'));
		}
		return false;
	}
	...
	public function delete( $params = array() ) {
		if (FramePyt::_()->getTable($this->_tbl)->delete($params)) {
	...
	public function insert( $data ) {
		...
		$id = FramePyt::_()->getTable($this->_tbl)->insert($data);
		if ($id) {
			return $id;
		}
		$this->pushError(FramePyt::_()->getTable($this->_tbl)->getErrors());
		return false;
	}
	public function updateById( $data, $id = 0 ) {
		...
		if ($id) {
			...
			$this->pushError(esc_html__('Empty or invalid ID', 'publish-your-table'));
		}
		return false;
	}
	public function update( $data, $where ) {
		$data = $this->_dataSave($data, true);
		if (FramePyt::_()->getTable($this->_tbl)->update($data, $where)) {
			return true;
		}
		$this->pushError(FramePyt::_()->getTable($this->_tbl)->getErrors());
		return false;
	...
	public function getFieldLists( $field, $key = false ) {
		$list = UtilsPyt::getArrayValue($this->fieldLists, $field, array(), 2);
		return $key === false ? $list : UtilsPyt::getArrayValue($list, $key);
	}
