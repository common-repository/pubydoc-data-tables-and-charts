...
class FieldPyt {
	 ...
	 */
	public $adapt = array( 'HtmlPyt' => '', 'dbFrom' => '', 'dbTo' => '' );
	/**
	...
	public function setValue( $value, $fromDB = false ) {
		if (isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB) {
			$value = FieldAdapterPyt::_($value, $this->adapt['dbFrom'], FieldAdapterPyt::DB);
		}
		$this->value = $value;
	}
	...
	public function displayValue() {
		...
		switch ($this->html) {
			case 'checkboxlist':
				$options = $this->getHtmlParam('OptionsPyt');
				$value = array();
				...
				if (empty($value)) {
					$value = esc_html__('N/A', 'publish-your-table');
				} else {
				...
				if ('' == $this->value) {
					...
					if (is_array($this->value)) {
						...
						if (!empty($options) && is_array($options)) {
							...
							$value = recImplodePyt('<br />', $valArr);
						} else {
							$value = recImplodePyt('<br />', $this->value);
						}
					} else {
						$value = $this->value;
					}
	...
	public function showValue() {
		HtmlPyt::echoEscapedHtml($this->displayValue());
	}
	...
	public function checkVarFromParam( $param, $element ) {
		return UtilsPyt::xmlAttrToStr($param, $element);
	}
	...
	public function prepareConfigOptions( $xml ) {
		...
		foreach ($config->params->param as $param) {
			...
			$helper = $this->checkVarFromParam($param, 'HelperPyt');
			$module = $this->checkVarFromParam($param, 'ModulePyt');
