...
if ((!isset($_SERVER['DOCUMENT_ROOT'])) OR (empty($_SERVER['DOCUMENT_ROOT']))) {
	if(isset($_SERVER['SCRIPT_FILENAME'])) {
		$_SERVER['DOCUMENT_ROOT'] = str_replace( '\\', '/', substr(sanitize_text_field($_SERVER['SCRIPT_FILENAME']), 0, 0-strlen(sanitize_text_field($_SERVER['PHP_SELF']))));
	} elseif(isset($_SERVER['PATH_TRANSLATED'])) {
		$_SERVER['DOCUMENT_ROOT'] = str_replace( '\\', '/', substr(str_replace('\\\\', '\\', sanitize_text_field($_SERVER['PATH_TRANSLATED'])), 0, 0-strlen(sanitize_text_field($_SERVER['PHP_SELF']))));
	} else {
		// define here your DOCUMENT_ROOT path if the previous fails (e.g. '/var/www')
		$_SERVER['DOCUMENT_ROOT'] = '/';
	}
}
$_SERVER['DOCUMENT_ROOT'] = str_replace('//', '/', sanitize_text_field($_SERVER['DOCUMENT_ROOT']));
...
if (!defined('K_PATH_URL')) {
	...
	if (isset($_SERVER['HTTP_HOST']) AND (!empty($_SERVER['HTTP_HOST']))) {
		...
		$k_path_url .= sanitize_text_field($_SERVER['HTTP_HOST']);
		$k_path_url .= str_replace( '\\', '/', substr(K_PATH_MAIN, (strlen(sanitize_text_field($_SERVER['DOCUMENT_ROOT'])) - 1)));
	}
