	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function fileGetContents($file) {
		...
		if ((strlen($file) > 1)
		    ...
		    $findroot = strpos($file, sanitize_text_field($_SERVER['DOCUMENT_ROOT']));
		    if (($findroot === false) || ($findroot > 1)) {
			$alt[] = htmlspecialchars_decode(urldecode(sanitize_text_field($_SERVER['DOCUMENT_ROOT']).$file));
		    }
		}
		//
		$protocol = 'http';
		...
		if (preg_match('%^(https?)://%', $url)
			...
			if (empty($urldata['query'])) {
				$host = $protocol.'://'.sanitize_text_field($_SERVER['HTTP_HOST']);
				if (strpos($url, $host) === 0) {
				    // convert URL to full server path
				    $tmp = str_replace($host, sanitize_text_field($_SERVER['DOCUMENT_ROOT']), $url);
		...
		if (isset($_SERVER['SCRIPT_URI'])
		    ...
		    $urldata = @parse_url(sanitize_text_field($_SERVER['SCRIPT_URI']));
