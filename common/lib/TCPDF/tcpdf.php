			...
			case 'img': {
				...
				if ($imgsrc[0] === '@') {
					...
					if (($imgsrc[0] === '/') AND !empty($_SERVER['DOCUMENT_ROOT']) AND ($_SERVER['DOCUMENT_ROOT'] != '/')) {
						// fix image path
						$findroot = strpos($imgsrc, sanitize_text_field($_SERVER['DOCUMENT_ROOT']));
						if (($findroot === false) OR ($findroot > 1)) {
							if (substr($_SERVER['DOCUMENT_ROOT'], -1) == '/') {
								$imgsrc = substr(sanitize_text_field($_SERVER['DOCUMENT_ROOT']), 0, -1).$imgsrc;
							} else {
								$imgsrc = sanitize_text_field($_SERVER['DOCUMENT_ROOT']).$imgsrc;
							}
						}
				...
				if (!$clipping) {
					...
					if (preg_match('/^data:image\/[^;]+;base64,/', $img, $m) > 0) {
						...
						if (($img[0] == '/') AND !empty($_SERVER['DOCUMENT_ROOT']) AND ($_SERVER['DOCUMENT_ROOT'] != '/')) {
							$findroot = strpos($img, sanitize_text_field($_SERVER['DOCUMENT_ROOT']));
							if (($findroot === false) OR ($findroot > 1)) {
								if (substr($_SERVER['DOCUMENT_ROOT'], -1) == '/') {
									$img = substr(sanitize_text_field($_SERVER['DOCUMENT_ROOT']), 0, -1).$img;
								} else {
									$img = sanitize_text_field($_SERVER['DOCUMENT_ROOT']).$img;
								}
							}
						}
						$img = urldecode($img);
