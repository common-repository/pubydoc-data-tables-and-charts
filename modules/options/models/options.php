...
class OptionsModelPyt extends ModelPyt {
	...
	public function saveGroup( $d = array() ) {
		if (isset($d['opt_values']) && is_array($d['opt_values']) && !empty($d['opt_values'])) {
			DispatcherPyt::doAction('beforeSaveOpts', $d);
			...
			$this->pushError(esc_html__('Empty data to save option', 'publish-your-table'));
		}

		return false;
	}
	private function _updateOptsInDb() {
		update_option(PYT_CODE . '_opts_data', $this->_values);
	}
	private function _loadOptValues() {
		if (!$this->_valuesLoaded) {
			$this->_values = get_option(PYT_CODE . '_opts_data');
