 ...
 */
if (!function_exists('strFirstUpPyt')) {
	function strFirstUpPyt( $str ) {
 ...
 */
if (!function_exists('dateToTimestampPyt')) {
	function dateToTimestampPyt( $date ) {
		if (empty($a)) {
			return false;
		}
		$a = explode(PYT_DATE_DL, $date);
 ...
 */
if (!function_exists('getRandNamePyt')) {
	function getRandNamePyt( $lenFrom = 6, $lenTo = 9 ) {
		$res = '';
...
if (!function_exists('importPyt')) {
	function importPyt( $path ) {
		if (file_exists($path)) {
			require($path);
			return true;
		}
...
if (!function_exists('setDefaultParamsPyt')) {
	function setDefaultParamsPyt( $params, $default ) {
...
if (!function_exists('importClassPyt')) {
	function importClassPyt( $class, $path = '' ) {
		if (!class_exists($class)) {
			if (!$path) {
				$classFile = lcfirst($class);
				if (strpos(strtolower($classFile), PYT_CODE) !== false) {
					$classFile = preg_replace('/' . PYT_CODE . '/i', '', $classFile);
				}
				$path = PYT_CLASSES_DIR . $classFile . '.php';
			}
			return importPyt($path);
		}
		return false;
	}
}
 ...
 */
if (!function_exists('toeGetClassNamePyt')) {
	function toeGetClassNamePyt( $class ) {
		$className = '';
		if (class_exists($class . strFirstUpPyt(PYT_CODE))) {
			$className = $class . strFirstUpPyt(PYT_CODE);
		} else if (class_exists(PYT_CLASS_PREFIX . $class)) {
			$className = PYT_CLASS_PREFIX . $class;
		} else {
			$className = $class;
		}
		return $className;
 ...
 */
if (!function_exists('toeCreateObjPyt')) {
	function toeCreateObjPyt( $class, $params ) {
		$className = toeGetClassNamePyt($class);
		$obj = null;
 ...
 */
if (!function_exists('redirectPyt')) {
	function redirectPyt( $url ) {
		if (headers_sent()) {
...
if (!function_exists('jsonEncodeUTFnormalPyt')) {
	function jsonEncodeUTFnormalPyt( $value, $ent = false ) {
		if (is_int($value)) {
			return (string) $value;   
		...
		if ($with_keys) {
			foreach ($value as $key => $v) {
				$result[] = jsonEncodeUTFnormalPyt((string) $key, $ent) . ':' . jsonEncodeUTFnormalPyt($v, $ent);    
			}
			return '{' . implode(',', $result) . '}';                
		} else {
			foreach ($value as $key => $v) {
				$result[] = jsonEncodeUTFnormalPyt($v, $ent);    
			}
			return '[' . implode(',', $result) . ']';
		}
	} 
 ...
 * @return array
 */
if (!function_exists('prepareParamsPyt')) {
	function prepareParamsPyt( &$d = array(), &$options = array() ) {
		if (!empty($d['params'])) {
			if (isset($d['params']['options'])) {
				$options = $d['params']['options'];
			}
			if (is_array($d['params'])) {
				$params = UtilsPyt::jsonEncode($d['params']);
		...
		if (isset($d['code'])) {
			if ('' == $d['code']) {
				$d['code'] = prepareFieldCodePyt($d['label']) . '_' . rand(0, 9999999);
			}
		}
		return $d;
	}
}
if (!function_exists('prepareFieldCodePyt')) {
	function prepareFieldCodePyt( $string ) {   
 ...
 */
if (!function_exists('recImplodePyt')) {
	function recImplodePyt( $glue, $array ) {
		$res = '';
		$i = 0;
		$count = count($array);
		foreach ($array as $el) {
